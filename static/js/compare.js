$(function () {
    function parseOfx (ofxString) {
        ...
        var stmttrn_list =
            doc.getElementsByTagName('OFX')[0]
            .getElementsByTagName('BANKMSGSRSV1')[0]
            .getElementsByTagName('STMTTRNRS')[0]
            .getElementsByTagName('STMTRS')[0]
            .getElementsByTagName('BANKTRANLIST')[0]
            .childNodes
        var ans = []
        stmttrn_list.forEach(function (stmttrn) {
            var dtposted = stmttrn.getElementsByTagName('DTPOSTED')[0].childNodes[0].nodeValue
            var trnamt = stmttrn.getElementsByTagName('TRNAMT')[0].childNodes[0].nodeValue
    ...
    function compare (ofxString, appBills) {
        ...
        lookupKeys.forEach(function (key) {
            ...
            keyBills.forEach(function (bill) {
                $('#entries').addClass('table table-bordered').append(
    ...
    function handleFiles (files) {
        ...
        reader.onload = function (e) { initCompare(e.target.result) }
